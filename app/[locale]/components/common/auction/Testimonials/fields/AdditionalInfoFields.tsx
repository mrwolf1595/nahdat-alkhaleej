...
const AdditionalInfoFields: React.FC<AdditionalInfoFieldsProps> = ({
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <label className="block text-sm font-medium text-gray-700">Would you use our platform again?</label>
        <div className="flex gap-3">
          {['Yes', 'Maybe', 'No'].map((option, index) => (
            <motion.button
              key={option}
              type="button"
              className={`flex-1 p-4 rounded-lg border ${getReturningButtonStyle(option)} transition-all`}
              onClick={() => handleReturningSelect(option)}
              ...
              animate={{ 
                opacity: 1, 
                y: 0,
                scale: formData.returning === option ? [1, 1.05, 1] : 1 
              }}
              transition={{ 
                duration: 0.4,
                delay: index * 0.1 + 0.1
      ...
      >
        <label className="block text-sm font-medium text-gray-700">Suggestions for improvement (optional)</label>
        <textarea
          name="improvement"
          ...
          placeholder="What could we do better? Your suggestions help us improve."
        />
      </motion.div>
      ...
      >
        <label className="block text-sm font-medium text-gray-700">Upload your avatar (optional)</label>
        <motion.div 
          ...
          <label htmlFor="avatar-upload" className="cursor-pointer block">
            {formData.avatar ? (
              ...
              >
                ...
                <p className="text-sm text-gray-700 font-medium">File selected: {formData.avatar.name}</p>
              ...
              >
                ...
                <p className="text-gray-600 mt-2">Click to upload your photo</p>
